...
// v36: v31 base (working) + diagnostic method hooks on background queue
...
static void delayed_hook(void) {
    ...
    NSLog(@"[WizardBypass] Delayed hook complete (v36 — crypto bypass active)");

    // v36: Diagnostic hooks on key Wizard methods to trace validation flow
    Class wksClass = objc_getClass("Wksahfnasj");
    if (wksClass) {
        NSString *wksNames[] = {@"paDJSAFBSANC", @"jsafbSAHCN", @"dgshdsfyewrh"};
        for (int i = 0; i < 3; i++) {
            SEL sel = sel_registerName([wksNames[i] UTF8String]);
            Method m = class_getInstanceMethod(wksClass, sel);
            if (m) {
                __block IMP origImp = method_getImplementation(m);
                __block SEL origSel = sel;
                __block NSString *name = wksNames[i];
                ...
                NSLog(@"[WizardBypass] Hooked Wksahfnasj::%@", wksNames[i]);
            }
        }
    }
    Class abvClass = objc_getClass("ABVJSMGADJS");
    if (abvClass) {
        NSString *abvNames[] = {@"PADSGFNDSAHJ", @"IKAFHFDSAJ", @"ASFGAHJFAHS", @"MdhsaJFSAJ"};
        for (int i = 0; i < 4; i++) {
            SEL sel = sel_registerName([abvNames[i] UTF8String]);
            Method m = class_getInstanceMethod(abvClass, sel);
            if (m) {
                __block IMP origImp = method_getImplementation(m);
                __block SEL origSel = sel;
                __block NSString *name = abvNames[i];
                IMP newImp = imp_implementationWithBlock(^void(id self) {
                ...
                method_setImplementation(m, newImp);
                NSLog(@"[WizardBypass] Hooked ABVJSMGADJS::%@", abvNames[i]);
            }
        }
    }
...
static void wizard_bypass_init(void) {
    NSLog(@"[WizardBypass] ========================================");
    NSLog(@"[WizardBypass] v36 (v31 base + diagnostics) - EARLY INIT");
    NSLog(@"[WizardBypass] ========================================");
